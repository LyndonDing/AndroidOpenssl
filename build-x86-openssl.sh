...
# try to pick it up with the value of _ANDROID_NDK_ROOT below. If
 ANDROID_NDK_ROOT="/Users/Lyndon/Library/Android/sdk/ndk-bundle"
...
./config shared no-ssl2 no-ssl3 no-comp no-hw no-engine --openssldir=/usr/local/ssl/$ANDROID_API/x86

make depend
sudo -E make CALC_VERSIONS="SHLIB_COMPAT=; SHLIB_SOVER=" MAKE="make -e" all
mkdir -p $OPENSSLDIR/lib
echo "place-holder make target for avoiding symlinks" >> $OPENSSLDIR/lib/link-shared
sudo -E make SHLIB_EXT=.so install_sw
rm $OPENSSLDIR/lib/link-shared
#sudo -E make install CC=$ANDROID_TOOLCHAIN/arm-linux-androideabi-gcc RANLIB=$ANDROID_TOOLCHAIN/i686-linux-android-ranlib
